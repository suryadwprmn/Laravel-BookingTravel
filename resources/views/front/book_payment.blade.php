...
<body class="font-poppins text-black">
    <section id="content" class="max-w-[640px] w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[120px]">
        ...
        <form method="POST" action="{{route('front.book_payment_store', $packageBooking->id)}}" class="flex flex-col gap-8" enctype="multipart/form-data">
          @csrf
            @method('PATCH')
            ...
            <div class="bg-white p-4 rounded-[26px] flex items-center gap-3">
              <div class="w-[72px] h-[72px] flex shrink-0 rounded-xl overflow-hidden">
                <img src="{{ Storage::url($packageBooking->package_tour->thumbnail) }}" class="w-full h-full object-cover object-center" alt="thumbnail">
              </div>
              <div class="flex flex-col gap-1">
                <p class="font-semibold text-sm tracking-035 leading-[22px]">{{$packageBooking->package_tour->name}}</p>
            ...
            <div class="bg-white p-[16px_24px] rounded-[26px] flex flex-col gap-4">
              <div class="flex flex-col gap-1">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm tracking-035 leading-[22px]">{{$packageBooking->package_bank->bank_name}}</span>
                </div>
              </div>
              <div class="flex flex-col gap-1">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm tracking-035 leading-[22px]">{{$packageBooking->package_bank->bank_account_name}}</span>
                </div>
              </div>
              <div class="flex flex-col gap-1">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span id="bank-number" class="text-sm tracking-035 leading-[22px]" data-value="7342333056">{{$packageBooking->package_bank->bank_account_number}}</span>
